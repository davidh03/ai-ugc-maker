...
export function createJob(input) {
  ...
  const { brief, durationSec, style = 'product', music = false, voiceover = false, voiceoverProvider = 'openai-tts', voiceoverVoice = 'coral', voiceoverOffsetSec = 0, durationMode = 'fixed', model = '', reasoningEffort = '', assets = [], composer, provider } = normalized;
...
${input.changeRequest.trim()}`;
  const child = createJob({ ...changeSet.settingsDiff.after, brief: revisionBrief });
