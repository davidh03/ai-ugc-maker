...
// Guards against a malformed/hallucinated cropBox from the vision model
// (out-of-range fractions, a box that runs off the edge of the image, a
// zero-area box) before it ever reaches the cropper.
export function isFractionalBox(box) {
...
async function analyzeImageWithLuna(filePath, assetId) {
  ...
  const prompt = 'Analyze the attached image for a video editor. Return ONLY valid JSON with these keys: description (string), objects (array of strings), visualRole (one of hero, logo, person, background, detail, outro, supporting), recommendedTiming (string), recommendedMotion (string), confidence (number 0 to 1), cropBox (an object {x, y, width, height}, each a fraction from 0 to 1 of the full image, giving the smallest rectangle that contains everything a viewer actually needs to see — e.g. for a screenshot with an unrelated list, sidebar, or empty margin next to the one thing the scene is actually about, bound tightly around just that relevant part; for a logo, photo, or image where the whole frame is already meaningful, return the full frame {x:0,y:0,width:1,height:1}). Do not include markdown fences or extra text.';
  try {
