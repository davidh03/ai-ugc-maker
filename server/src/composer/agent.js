...
import { mkdirSync, writeFileSync, existsSync, readFileSync, copyFileSync, statSync, unlinkSync } from 'node:fs';
...
export function runOnce(binary, args, jobDir, model) {
  return new Promise((resolve, reject) => {
    try { if (existsSync(path.join(jobDir, 'index.html'))) unlinkSync(path.join(jobDir, 'index.html')); } catch { /* stale artifact will fail validation below */ }
    ...
    child.on('close', code => code === 0 && existsSync(outputPath) ? finish(null, jobDir) : finish({ code: code ?? 'unknown', output: (stderr || stdout).slice(0, 20000) }));
...
export const agentComposer = {
  async compose(job) {
    ...
    const staged = stageAssets(job, jobDir); const sourceCompositionPath = job.parentJobId ? path.join(DATA_DIR, "jobs", job.parentJobId, "index.html") : ""; const sourceCompositionAvailable = Boolean(sourceCompositionPath && existsSync(sourceCompositionPath)); if (sourceCompositionAvailable) { copyFileSync(sourceCompositionPath, path.join(jobDir, "source-index.html")); if (job.revisionContext && job.revisionContext.deterministic) { const patched = patchComposition(readFileSync(sourceCompositionPath, "utf8"), job); if (patched.changed) { const deterministicPrompt = `DETERMINISTIC REVISION PATCH\nInstruction: ${job.revisionContext.instruction || job.revisionInstruction || ""}\nTargets: ${JSON.stringify(job.revisionTargets || [])}\nAsset bindings: ${JSON.stringify(job.assetBindings || [])}\nOperations: ${JSON.stringify(patched.changes || [])}\nPreserve every unspecified source region.`; job.promptProvenance = finalizePromptProvenance(job.promptProvenance, deterministicPrompt); writeFileSync(path.join(jobDir, "prompt.txt"), deterministicPrompt); writeFileSync(path.join(jobDir, "index.html"), patched.html); job.revisionPatch = { applied: true, changes: patched.changes, reason: patched.reason }; return jobDir; } } } const dimensions = job.style === 'social' ? '1080x1920 vertical' : '1920x1080 landscape'; let prompt = `Create index.html from the complete user brief: ${job.brief}. The requested output duration is exactly ${job.durationSec} seconds; this setting is authoritative. If the brief contains longer timecodes or a longer stated duration, compress or retime every scene to fit exactly within ${job.durationSec} seconds and finish the end card inside that limit. ${job.durationSec}s ${job.style || 'product'} video in ${dimensions}. Preserve all user requirements, including titles, notes, camera directions, visual details, on-screen copy, and calls to action. Use GSAP animations. If you animate a pan, zoom, or scale on a screenshot or its container (a "camera" move), any highlight/annotation/callout box or label meant to point at specific content inside that screenshot MUST be nested inside that exact same transformed element (so it inherits the same transform automatically) or be given an identical counter-animation in the GSAP timeline. Never give a screenshot a moving camera and then draw a separately-positioned sibling annotation with static coordinates on top of it — the annotation will visibly drift off its target as the camera moves, since its fixed pixel position was only correct for the frame's untransformed starting state. Never render the same text string (a stat, a label, a headline) twice in nearly the same position as a stylistic effect — e.g. two overlapping copies for a glow/duplicate-outline look, or a "count up" effect built from two separate elements showing the old and new value at once. This reliably renders as a visible ghosting/double-exposure artifact, not the intended effect. If a value needs to change on screen, swap it with a single element whose content updates cleanly (a gsap.set() call on textContent, or one crossfade where the outgoing element reaches exactly opacity 0 before or as the incoming one reaches opacity 1) — never two simultaneously-present text layers at the same position. When a headline has multiple lines or a styled sub-span (e.g. a span used for emphasis), give every line and span the same opacity/transform treatment in its entrance animation unless a different final state is intentional — an entrance that finishes leaving one line still faint while a sibling line is fully opaque is a bug, not a style choice. Include data-composition-id, data-width, data-height, class=clip, data-track-index, data-start, data-duration attributes on divs. Add window.__timelines. Create a natural voiceover script that matches the actual scene order and on-screen text, then embed it as window.__voiceoverScript = "<the exact spoken words, as one plain double-quoted JSON string>". It must be a plain string — not an object, not JSON with segments/timing/fullText fields, just the narration text itself. Voiceover direction: ${narrationDirection(job.style)} Do not put URLs, research notes, production instructions, or visual descriptions into the spoken script.`;
