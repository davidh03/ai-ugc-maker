...
function GenerationCard({ job, onDelete, deleting, deleteError }) {
...
export default function Generations() {
  ...
  const counts = useMemo(() => ({ all: jobs.length, done: jobs.filter(job => job.status === 'done').length, active: jobs.filter(job => job.status === 'queued' || job.status === 'running').length, failed: jobs.filter(job => job.status === 'failed' || job.status === 'cancelled').length }), [jobs]);
