...
export default function PromptForm({ onSubmit, loading }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ brief: brief.trim(), durationSec, style, music, agent, model: agent !== 'none' ? model : undefined, assets });
    setBrief('');
    setAssets([]);
  };

  const sel = { padding: '4px 8px', backgroundColor: '#1a1a1a', color: '#fff', border: '1px solid #333', borderRadius: 4, fontSize: 13 };

  return (
    <form onSubmit={handleSubmit} style={{ marginBottom: 24 }}>
      <AssetUpload onAssetsChange={setAssets} />
      <textarea value={brief} onChange={e => setBrief(e.target.value)} placeholder="Describe your video..." rows={3}
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <label style={{ fontSize: 13, color: '#aaa' }}>
          Duration: <input type="number" value={durationSec} onChange={e => setDurationSec(Number(e.target.value))} min={1} max={180}
            style={{ width: 60, marginLeft: 6, padding: '4px 8px', backgroundColor: '#1a1a1a', color: '#fff', border: '1px solid #333', borderRadius: 4 }} /> s
        </label>
